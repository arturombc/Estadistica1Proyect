...
```{r}
n <- length(DF$LLAMADAS_IN_TOT)
m <- max(log(DF$LLAMADAS_IN_TOT), na.rm = T)
mm <- floor(m)
hist(ceiling(log(DF$LLAMADAS_IN_TOT)), probability = T)
lines((-1:mm), dbinom(-1:mm, mm, 0.60), type = "h", col = "red")
```
...
Definimos un experimento $\epsilon_2$ determinado por $\{\Omega_2, \mathcal{F}_2, \mathbb{P}_2\}$ que consiste en que elijo una persona de la muestra y anoto la cantidad de veces que realizó una llamada durante el mes.
...
Si se desea hallar la probabilidad de que si al repetir el experimento $\epsilon_2$ 100 veces, por lo menos 10 de ellos hayan llamado menos de 50 veces durante el mes, debemos plantear un modelo de tipo binomial, donde:
