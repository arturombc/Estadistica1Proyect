...
Definimos un experimento $\epsilon_3$ determinado por $\{\Omega_3, \mathcal{F}_3, \mathbb{P}_3\}$ como la cantidad de veces que te llaman durante un mes.
...
Definimos $\lambda$ como $\bar{X_3}$ que es igual a `r mean(X_3)`

Tomaremos este valor aún sabiendo que `r mean(X_3)` difiere de `r var(X_3)` considerablemente.
