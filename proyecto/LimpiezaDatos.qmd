...
```{r}
orden_corregido <- c(
  "ID", "CHIP", "MARCA", "MODELO", "OS", "DATOS_4G", "DATOS_3G", "DATOS_2G", "DATOS_TOTAL",
  "TIEMPO_4G", "TIEMPO_3G", "TIEMPO_2G", "DPTO_DATOS", "DPTO_TIEMPO_LLAMADAS",
  "TIEMPO_LLAMADAS_IN_TOT", "TIEMPO_LLAMADAS_IN_SEM", "TIEMPO_LLAMADAS_IN_FINDE",
  "LLAMADAS_IN_TOT", "LLAMADAS_IN_SEM", "LLAMADAS_IN_FINDE", "TIEMPO_LLAMADAS_OUT_TOT",
  "TIEMPO_LLAMADAS_OUT_SEM", "TIEMPO_LLAMADAS_OUT_FINDE", "LLAMADAS_OUT_TOT", "LLAMADAS_OUT_SEM",
  "LLAMADAS_OUT_FINDE", "CONTACTOS_LLAMADAS_IN_TOT", "CONTACTOS_LLAMADAS_OUT_TOT",
  "DIAS_LLAMADAS_IN_TOT", "DIAS_LLAMADAS_OUT_TOT", "CONTACTOS_TOT", "CONTACTOS_TOT_5"
)

data4 <- data4[, orden_corregido]
```


```{r}
write.csv(data4, "DataLimpia.csv", row.names = FALSE)
round(sum(is.na(data4))/(nrow(data4)*ncol(data4))*100 , digits = 2)
```
```{r}
str(data4)
```
