...
```{r}
ggplot(DF,aes(x=PERIODO,y=DATOS_TOTAL,fill=OS),xlabel=)+geom_col(position="dodge")+ggtitle("TRAFICO DE DATOS TOTAL Y TECNOLOGIA DEL CHIP") + theme(plot.title = element_text(hjust = 0.5)) +ylab("TRAFICO DE DATO(Mb)")+labs(fill="TECNOLOGIA DEL CHIP")+theme(axis.text = element_text(angle = 90))
```
...
```{r}
ggplot(data = DF, aes(y = DF$CALLS_IN_TOT, x = DF$AIRTIME_IN_TOT)) +
  geom_point()  + labs(x="LLAMADAS ENTRANTES TOTALES", y="TIEMPO TOTAL DE LLAMADAS RECIBIDAS", title="Gráfico 4: dispersión de compras diarias vs gasto diario con billeteras digitales")
```
...
ggplot(DF,aes(x=PERIODO,y=DATOS_TOTAL,fill=categorica_factor),xlabel=)+geom_col(position="dodge")+ggtitle("TRAFICO DE DATOS TOTAL Y TECNOLOGIA DEL CHIP") + theme(plot.title = element_text(hjust = 0.5)) +ylab("TRAFICO DE DATO(Mb)")+labs(fill="TECNOLOGIA DEL CHIP")+theme(axis.text = element_text(angle = 90))

```

**Comparación de llamadas de días laborables y no laborales:**

```{r}
# Calcular el promedio de llamadas en días laborables y no laborables
promedio_calls_week <- mean(DF$CALLS_OUT_WEEK)
promedio_calls_weekd <- mean(DF$CALLS_OUT_WEEKD)

# Crear un gráfico de barras para comparar llamadas en días laborables y no laborables
barplot(c(promedio_calls_week, promedio_calls_weekd), 
        names.arg = c("Días Laborables", "Días No Laborables"),
        main = "Promedio de Llamadas Salientes por Tipo de Día",
        ylab = "Promedio de Llamadas",
        col = c("lightblue", "lightcoral"))

# Imprimir el promedio de llamadas en días laborables y no laborables
cat("Promedio de Llamadas en Días Laborables:", promedio_calls_week, "\n")
cat("Promedio de Llamadas en Días No Laborables:", promedio_calls_weekd, "\n")
```
