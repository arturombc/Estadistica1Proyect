...
Ya que tenemos estas primeras impresiones, podemos intentar entender cómo es que se comporta el consumo de datos 4G, 3G y 2G por cada equipo, además podemos dividir esta vista en periodos para tener una interpretación holística.
...
ggplot(DF, aes(CHIP,DATOS_TOTAL/1000, fill=CHIP)) + 
  ...
  #ylim(0,1000) + 
  ggtitle("Uso de red de usuarios 4G por periodo") +
  labs(x = NULL, y = "DATOS EN GB")

boxplot(Valor ~ Indice, data = data, 
        main = "Boxplot Indexado",
        xlab = "Índice", ylab = "Valor")
```

```{r}
cuatroG<-filter(DF, CHIP=="4G")
boxplot(cuatroG$DATOS_TOTAL~cuatroG$PERIODO,data=cuatroG,col="green",horizontal=FALSE,Notch=TRUE,outline=F)
```

```{r}
tresG<-filter(DF, CHIP=="3G")
boxplot(tresG$DATOS_TOTAL~tresG$PERIODO,data=tresG,col="green",horizontal=FALSE,Notch=TRUE,outline=F)
```

```{r}
dosG<-filter(DF, CHIP=="2G")
boxplot(dosG$DATOS_TOTAL~dosG$PERIODO,data=dosG,col="green",horizontal=FALSE,Notch=TRUE,outline=F)
```

```{r}
DF$DATOS_4G %>% median() / 1000
```

```{r}
DF %>% filter(DATOS_TOTAL == 0)
```

Podemos decir entonces que durante un mes, el promedio de uso de datos 4G, 3G y 2G son `r mean(DF$DATOS_TOTAL[DF$CHIP == "4G"])`

```{r}
mean(DF$DATOS_TOTAL[DF$CHIP == "4G"], na.rm = TRUE) / 1000
median(DF$DATOS_TOTAL[DF$CHIP == "4G"], na.rm = TRUE) / 1000
table(DF$CHIP)
sum(DF$DATOS_TOTAL[DF$CHIP == "4G"], na.rm = TRUE) / sum(!is.na(DF$DATOS_TOTAL[DF$CHIP == "4G"]))
mean(DF$DATOS_TOTAL[DF$CHIP == "4G"], na.rm = TRUE)
```
...
**Gráfico 6: Cantidad total de llamadas entrantes y Tiempo total de llamadas entrantes por usuario**

```{r}
resultado <- DF %>%
group_by(ID) %>%
summarise(TIEMPO_LLAMADAS_IN_TOTAL_PERIODOS = sum(TIEMPO_LLAMADAS_IN_TOT,na.rm=TRUE),LLAMADAS_IN_TOTAL_PERIDOS=sum(LLAMADAS_IN_TOT,na.rm=TRUE))
print(resultado)

ggplot(resultado, aes(x = resultado$LLAMADAS_IN_TOTAL_PERIDOS, y = resultado$TIEMPO_LLAMADAS_IN_TOTAL_PERIODOS)) +
  geom_point(shape = 16, color = "lightblue") +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  labs(x = "Cantidad de llamadas registradas en total de 9 meses ", y = "tiempo de llamadas regitradas en total de 9 meses",
       title = "Cantidad total de llamadas registradas vs tiempo total de llamadas regitradas")
```

**Gráfico 7: Cantidad total de llamadas salientes y Tiempo total de llamadas salientes por usuario**

```{r}
resultado1 <- DF %>%
group_by(ID) %>%
summarise(TIEMPO_LLAMADAS_OUT_TOTAL_PERIODOS = sum(TIEMPO_LLAMADAS_OUT_TOT,na.rm=TRUE),LLAMADAS_OUT_TOTAL_PERIDOS=sum(LLAMADAS_OUT_TOT,na.rm=TRUE))
print(resultado1)

ggplot(resultado1, aes(x = resultado1$LLAMADAS_OUT_TOTAL_PERIDOS, y = resultado1$TIEMPO_LLAMADAS_OUT_TOTAL_PERIODOS)) +
  geom_point(shape = 16, color = "lightblue") +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
