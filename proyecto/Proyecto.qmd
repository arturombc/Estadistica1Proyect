...
```{r}
cor(DF$LLAMADAS_OUT_TOT, DF$TIEMPO_LLAMADAS_OUT_TOT,use = "complete.obs")
```

El coeficiente de correlación de 0.5590268 indica una correlación positiva moderada entre la cantidad total de llamadas salientes y el tiempo total de llamadas salientes. Esto significa que, en promedio, un aumento en las llamadas salientes está asociado con un aumento en el tiempo total de esas llamadas, pero la relación no es extremadamente fuerte ni perfecta.

**Ecuación de regresión lineal:**

```{r}
lm(TIEMPO_LLAMADAS_OUT_TOTAL_PERIODOS  ~LLAMADAS_OUT_TOTAL_PERIODOS, data = DF_NEW11)
```
...
>>>>>>> ccd11319933fa2793ec699d7f1ec732bb560807c

A partir de diciembre de 2018 y hasta mayo de 2019, se observa una relativa estabilidad en la cantidad de días de llamadas, con una mediana constante de 29 días y una mínima variación en los cuartiles. La media cercana a la mediana sugiere una distribución que se asemeja a una normal, y la baja desviación estándar y coeficiente de variación indican que la dispersión en los datos es limitada en comparación con la media.

A pesar de las fluctuaciones en los valores máximos y mínimos, el rango intercuartil de 6 días señala que la mayor parte de los períodos tuvo una cantidad de días de llamadas muy similar, lo que respalda la idea de estabilidad. La baja desviación estándar y el coeficiente de variación cercano a cero confirman una dispersión limitada en comparación con la media, lo que indica que los datos están relativamente concentrados alrededor de la media.

No obstante, es fundamental recordar que estos patrones de estabilidad pueden ser influenciados por factores externos no reflejados en los datos estadísticos. Para una interpretación completa, es esencial considerar el propósito de las llamadas y el contexto detrás de la falta de datos en los primeros tres meses de 2018, que podría estar relacionada con problemas de registro o cambios en la recopilación de datos en ese período.
...
Sin embargo, para una interpretación completa, es esencial considerar el contexto detrás de este aumento en el consumo de datos. Diversos factores podrían influir en esta tendencia, como el lanzamiento de nuevas aplicaciones, cambios en los planes de datos, eventos estacionales o modificaciones en el comportamiento de los usuarios. Comprender la razón detrás de este incremento y cualquier evento o circunstancia que pueda explicarlo es crucial para una interpretación más precisa. En resumen, los datos sugieren un incremento constante en el consumo de datos móviles, pero para una comprensión completa, es esencial analizar el contexto y los factores adicionales que pueden estar contribuyendo a este aumento en el uso de datos móviles.
