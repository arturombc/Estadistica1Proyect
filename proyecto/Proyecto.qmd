...
>>>>>>> 0fd2878f6d64457ecde1dcf4bba94fcf04da81b1
:::
...
[terrible el link][link-1]

![][img-1]

- 2G: Permite solamente llamadas y mensajes. Generalmente se usan
...
```{r}
ggplot(DF, aes(x = DF$LLAMADAS_IN_TOT, y = DF$TIEMPO_IN_TOT)) +
...
```{r}
lm(DF$TIEMPO_IN_TOT~DF$LLAMADAS_IN_TOT,data=DF)
```
...
promedio_tiempo_week <- mean(DF$TIEMPO_IN_SEM)
promedio_tiempo_weekd <- mean(DF$TIEMPO_IN_FINDE)
...
```{r}
ggplot(DF, aes(x = DF$LLAMADAS_OUT_TOT, y = DF$TIEMPO_OUT_TOT)) +
  geom_point(shape = 16, color = "pink") +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  labs(x = "Cantidad de llamadas", y = "Tiempo en total",
...
```{r}
lm(DF$TIEMPO_OUT_TOT~DF$LLAMADAS_OUT_TOT,data=DF)
```
...
```{r}
# Calculamos el promedio del tiempo total de llamadas en días laborables y no laborables
promedio_tiempo_week <- mean(DF$LLAMADAS_OUT_SEM)
promedio_tiempo_weekd <- mean(DF$LLAMADAS_OUT_FINDE)
