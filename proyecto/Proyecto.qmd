...
La duración y la cantidad de llamadas entrantes es una forma de conocer las necesidades, preferencias y hábitos de consumo de los usuarios. Por ejemplo, si un usuario recibe muchas llamadas cortas, puede indicar que tiene problemas de conexión, o que prefiere comunicarse por otros medios. Por el contrario, si un usuario recibe pocas llamadas largas, puede indicar que tiene una buena cobertura y un buen servicio, o que le gusta hablar mucho por teléfono.

Por lo tanto, conocer mejor al cliente a través de su comportamiento y preferencias puede ayudar a las empresas de telefonía móvil a ofrecer planes más personalizados y adaptados a cada usuario, esto convierte este análisis en pilar fundamental para optimizar el servicio al cliente.
...
Podemos anticipar que esto implica que los usuarios con chip 4G tengan un mayor tiempo total de llamadas entrantes que los demás debido a la alta velocidad y calidad de conexión a red que este ofrece. Asimismo, debido a que Android tiene una mayor compatibilidad en comparación con otros sistemas operativos, se puede esperar que los usuarios de Android experimenten un mayor tiempo total de llamadas entrantes.

En este contexto, utilizando un gráfico de dispersión podremos verificar la linealidad entre la cantidad de llamadas entrantes y el tiempo total de llamadas entrantes por usuario.

**Gráfico 6: Cantidad de llamadas entrantes y Tiempo total de llamadas entrantes por usuario**

```{r}
ggplot(DF, aes(x = DF$LLAMADAS_IN_TOT, y = DF$TIEMPO_LLAMADAS_IN_TOT)) +
  geom_point(shape = 16, color = "lightgreen") +
  geom_smooth(method = "lm", se = FALSE, color = "darkgreen") +
  labs(x = "Cantidad de llamadas entrantes", y = "Tiempo total de llamadas entrantes",
       title = "Cantidad de llamadas entrantes y Tiempo total de llamadas entrantes") +
  theme(
...
La línea de tendencia (en color verde) trazada en el gráfico, nos muestra lo que mencionábamos anteriormente, que a medida que aumenta la cantidad de llamadas entrantes, el tiempo total de llamadas entrantes también tiende a aumentar en promedio. Sin embargo, si nos enfocamos en cada usuario, representados por los puntos en el gráfico, vemos que algunos usuarios recibieron una cantidad considerable de llamadas con poco duración mientras que otros estuvieron más tiempo en una menor cantidad de llamadas. Esto demuestra que no siempre una mayor cantidad de llamadas entrantes se traduce en un mayor tiempo total de llamadas entrantes.
...
```{r}
colores <- colorRampPalette(c("lightblue", "darkblue"))(length(unique(DF$CHIP)))

ggplot(DF, aes(x = CHIP, y = TIEMPO_LLAMADAS_IN_TOT, fill = CHIP)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = colores) +
  labs(
    x = "Tecnología de Chip",
    y = "Tiempo total de llamadas recibidas",
    title = "TIEMPO TOTAL DE LLAMADAS RECIBIDAS Y TECNOLOGÍA DEL CHIP"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5),
...
Se puede ver que los usuarios que utilizan la tecnología 4G del chip reciben más llamadas en promedio. Esto se debe a que la tecnología 4G ofrece una conexión más rápida y estable a internet y a la red móvil en comparación con las tecnologías más antiguas, como 2G o 3G. En ese sentido, los usuarios con tecnología 4G están más disponibles y dispuestos a recibir llamadas, ya que experimentan menos problemas de conectividad.

```{r}
colores <- colorRampPalette(c("lightcoral", "darkred"))(length(unique(DF$OS)))

ggplot(DF, aes(x = OS, y = TIEMPO_LLAMADAS_IN_TOT, fill = OS)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = colores) +
  labs(
    x = "Tipo de sistema operativo",
    y = "Tiempo total de llamadas recibidas",
    title = "TIEMPO TOTAL DE LLAMADAS RECIBIDAS Y SISTEMA OPERAITVO"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5),
...
Se puede ver que los usuarios que tienen el sistema operativo Android reciben más llamadas en promedio en comparación con otros. Esto se debe a que la mayoría de los usuarios tienen Android, por lo que aumenta la probabilidad de que los usuarios de Android se comuniquen entre sí.
...
La elección del plan de telefonía adecuado es crucial para que las empresas comprendan mejor a sus clientes. Cada llamada, mensaje y uso de datos proporciona información valiosa sobre los hábitos de comunicación y preferencias de los clientes. Al seleccionar el plan correcto, las empresas pueden recopilar datos relevantes que les permiten personalizar sus estrategias de marketing y servicios, lo que se traduce en una mayor satisfacción del cliente y relaciones comerciales más sólidas. Esta oportunidad estratégica se aplica a empresas de todos los tamaños y representa una forma efectiva de fortalecer las relaciones con los clientes y optimizar el éxito empresarial a través del conocimiento más profundo de sus intereses y comportamientos.
...
Al analizar la relación entre la duración de las llamadas salientes y el periodo de tiempo en el año 2019, se destaca una disparidad significativa en las barras de color rosa, que representan dispositivos con sistema operativo Android. Estos datos indican que los usuarios de dispositivos Android tienden a hablar por teléfono durante períodos más largos en comparación con otros sistemas operativos móviles. Esta información es valiosa para la toma de decisiones estratégicas en cuanto a la oferta de planes de telefonía.

En consecuencia, se podría argumentar que el público principal al que debería estar destinado este plan de telefonía sería aquel que utiliza dispositivos Android. Esto no solo se basa en la duración de las llamadas, sino también en la adaptabilidad del plan para satisfacer las necesidades y preferencias de este grupo demográfico específico. Además, sería beneficioso realizar un análisis más detallado para comprender las preferencias de uso de datos y mensajes de este público, con el fin de ofrecer un plan completo que se ajuste a todas sus necesidades de comunicación y que garantice la satisfacción del cliente.
...
El análisis de la relación entre el número de llamadas salientes y el periodo de tiempo en 2019 muestra una clara disparidad, con las barras de color rosa que representan dispositivos con sistema operativo Android sobresaliendo al registrar un considerable volumen de llamadas en comparación con otras plataformas. Esto indica que el público objetivo ideal para este plan de telefonía debería centrarse en usuarios de Android, ya que son quienes más utilizan el servicio de llamadas. Esta observación ofrece una valiosa dirección estratégica para las empresas de telecomunicaciones, permitiéndoles adaptar sus servicios y ofertas para satisfacer las necesidades de este grupo demográfico y, así, brindar una experiencia de comunicación óptima.
...
El gráfico revela un patrón interesante en cuanto a la tecnología de los chips utilizados por los usuarios de diferentes sistemas operativos móviles. La mayoría de los clientes que utilizan Android, iOS y sin sistema operativo de teléfono (SIN TAC) parecen estar equipados con chips 4G, lo que sugiere que estos usuarios disfrutan de una conectividad más rápida y una experiencia de datos más avanzada.

Por otro lado, se observa que la mayoría de los usuarios de otros sistemas operativos están utilizando chips 2G. Esto podría deberse a que estos dispositivos pueden ser más antiguos o menos avanzados en términos de capacidades de red. Esto es importante para las empresas de telefonía móvil, ya que pueden utilizar esta información para adaptar sus ofertas y servicios. Por ejemplo, podrían ofrecer planes de datos 4G específicamente dirigidos a los usuarios de Android, iOS y SIN TAC, para aprovechar su mayor capacidad de conectividad. Al mismo tiempo, podrían considerar ofrecer actualizaciones de dispositivos a los usuarios de otros sistemas operativos que aún utilizan tecnología 2G para mejorar su experiencia de red y retenerlos como clientes satisfechos.

**Conclusión general:**

El público objetivo principal para el plan de telefonía debería centrarse en usuarios de dispositivos Android debido a su mayor uso de llamadas salientes, con una duración significativa, y su adopción de tecnología 4G que sugiere una mayor preparación para servicios de telefonía móvil avanzados. Sin embargo, es esencial adaptar el plan para satisfacer las preferencias y necesidades específicas de este grupo demográfico, incluyendo el uso de datos y mensajes. Además, se podría considerar estrategias para retener a usuarios de otros sistemas operativos que aún utilizan tecnología 2G, como ofrecer actualizaciones de dispositivos o planes de datos mejorados para mejorar su experiencia de red y mantenerlos como clientes satisfechos.
