...
ggplot(DF, aes(CHIP, DATOS_TOTAL/1000, fill = CHIP)) + 
  ...
  geom_hline(data = media_per
             , aes(yintercept = DATOS_TOTAL / 1000)
             , color = "orange"
             , linetype = "dashed") +
...
En esta gráfica se utilizó el límite en el eje _y_ como el tercer cuartíl más 1.5 veces la IQR de `DF$DATOS_TOTAL`, además se eliminó la visualización de datos atípicos para una visualización más limpia. Esto se realizó porque los valores atípicos son extremadamente diferentes en escala al compararlos con los datos más comunes; por ejemplo, el máximo valor en GB que se gastó en un periodo es `r round(max(DF$DATOS_TOTAL/1000), 2)`, a comparación del valor de la mediana que es `r round(median(DF$DATOS_TOTAL/1000), 2)`. Debido a que este comportamiento, en algunos de los gráficos futuros se tomarán los valores dentro de un intervalo razonable (1.5 veces IQR).

```{r}
Q1 <- quantile(DF$DATOS_TOTAL, 0.25)/1000
Q3 <- quantile(DF$DATOS_TOTAL, 0.75)/1000
IQ <- IQR(DF$DATOS_TOTAL) / 1000
dat_tot_atipicos <- sum(DF$DATOS_TOTAL/1000 < (Q1 - 1.5 * IQ) | DF$DATOS_TOTAL/1000 > (Q3 + 1.5 * IQ))
dat_tot_atipicos
```
...
En este contexto, utilizando un gráfico de dispersión podremos verificar la linealidad entre la cantidad de llamadas entrantes y el tiempo total de llamadas entrantes por usuario. Sin embargo, ya que dentro de nuestro data frame los usuarios se repiten debido a la cantidad de periodos, decidimos comparar la cantidad de llamadas en total durante los 9 periodos, con el tiempo total de llamadas durante los 9 periodos; para ello se creó una nueva base de datos donde se agrupó el data frame según un mismo ID y se sumaron las respectivas variables de cantidad de llamadas y tiempo de llamadas.
...
ggplot(resultado, aes(x = resultado$LLAMADAS_IN_TOTAL_PERIODOS, y = resultado$TIEMPO_LLAMADAS_IN_TOTAL_PERIODOS)) +
...
Lo mismo sucede al utilizar un gráfico de dispersión entre la cantidad de llamadas salientes y el tiempo total de llamadas salientes por usuario.
...
ggplot(resultado1, aes(x = resultado1$LLAMADAS_OUT_TOTAL_PERIODOS, y = resultado1$TIEMPO_LLAMADAS_OUT_TOTAL_PERIODOS)) +
...
La línea de tendencia (en color azul) trazada en el gráfico, nos muestra lo que mencionábamos anteriormente, que a medida que aumenta la cantidad de llamadas entrantes, el tiempo total de llamadas entrantes también tiende a aumentar en promedio. Sin embargo, si nos enfocamos en cada usuario, representados por los puntos en el gráfico, vemos que algunos usuarios recibieron una cantidad considerable de llamadas con poca duración mientras que otros estuvieron más tiempo en una menor cantidad de llamadas. Esto demuestra que no siempre una mayor cantidad de llamadas entrantes se traduce en un mayor tiempo total de llamadas entrantes.
